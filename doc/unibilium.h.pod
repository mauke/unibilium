...
the terminfo database. It can parse compiled terminfo entries from memory or
files, access and modify their capabilities, and generate new terminfo entries.
...
integers or strings. It is currently implemented as a union of two members,
C<int i> and C<char *p>, but direct use of these is deprecated as the
representation will change in the future.

See L<unibi_var_from_num(3)>, L<unibi_var_from_str(3)>,
L<unibi_num_from_var(3)>, L<unibi_str_from_var(3)> instead.
